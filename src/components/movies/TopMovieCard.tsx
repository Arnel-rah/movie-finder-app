...
const TopMovieCard = ({ movie, index }: TopMovieCardProps) => {
  const displayGenres = movie.genre_ids && movie.genre_ids.length > 0 
    ? movie.genre_ids
        .slice(0, 2)
  ...
  return (
    <div className="shrink-0 flex items-center gap-4 w-72 md:w-80 group cursor-pointer">
      <span className="text-5xl md:text-7xl font-black text-white/10 group-hover:text-white/40 transition-colors italic min-w-15 text-center">
        {index + 1}
      </span>
