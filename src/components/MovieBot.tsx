      ...
      {isOpen && (
        <div className="absolute bottom-20 right-0 w-87.5 md:w-100 h-137.5 bg-[#0f0f0f] border border-white/10 rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          <div className="p-5 bg-linear-to-r from-[#00925d]/20 to-transparent border-b border-white/5 flex items-center justify-between">
            ...
            <button onClick={() => setIsOpen(false)} className="text-gray-500 hover:text-white transition-colors cursor-pointer">
              <X size={18} />
            </button>
          </div>
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-5 space-y-6 scroll-smooth">
            ...
            {chatLog.map((msg, i) => (
              <div key={i} className={`flex gap-3 ${msg.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
                <div className={`h-8 w-8 rounded-lg shrink-0 flex items-center justify-center ${msg.role === "user" ? "bg-white/10 text-gray-300" : "bg-[#00925d]/20 text-[#00925d]"}`}>
                  {msg.role === "user" ? <User size={16} /> : <Bot size={16} />}
                </div>
                <div className={`relative px-4 py-3 rounded-2xl text-sm leading-relaxed shadow-sm ${msg.role === "user" ? "bg-white/10 text-white rounded-tr-none" : "bg-white/3 text-gray-200 border border-white/5 rounded-tl-none"}`}>
          ...
          <div className="p-4 bg-white/2 border-t border-white/5">
            <div className="relative flex items-center">
              ...
              />
              <button onClick={askAI} disabled={isLoading} className="absolute right-2 p-2 text-[#00925d] hover:bg-[#00925d] hover:text-white rounded-xl transition-all disabled:opacity-30 cursor-pointer">
                <Send size={18} />
              </button>
            </div>
